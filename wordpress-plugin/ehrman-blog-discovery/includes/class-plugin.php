...
final class Plugin
{
    ...
    public function register(): void
    {
        ...
        update_option('ehrman_discovery_plugin_version', EBD_VERSION, false);
    ...
    {
        if ((string) get_option('ehrman_discovery_pages_version', '') !== EBD_VERSION) {
    ...
    public function handle_admin_import(): void
    {
        ...
        try {
            ...
            set_transient(
                'ehrman_discovery_notice_' . get_current_user_id(),
                array('success' => false, 'message' => $error->getMessage()),
                MINUTE_IN_SECONDS
            );
        }
    ...
    public static function status_data(): array
    {
        ...
        $import_state = is_array($import_status) && isset($import_status['state'])
            ? (string) $import_status['state']
            : 'not_imported';

        return array(
            'plugin_version' => EBD_VERSION,
            'schema_version' => (string) get_option('ehrman_discovery_schema_version', EBD_SCHEMA_VERSION),
            'database_connected' => '1' === (string) $connection_check,
