...
final class Post_Ingestion_Service {
	 ...
	 *
	 * @var Post_Ingestion_Review_Workflow
	 */
	private Post_Ingestion_Review_Workflow $review_workflow;
	 ...
	 * @param Post_Ingestion_Review_Workflow|null     $review_workflow    Optional review workflow.
	 */
	public function __construct(
		...
		?Post_Ingestion_Review_Workflow $review_workflow = null
	) {
		...
		$this->review_workflow     = $review_workflow ?? new Post_Ingestion_Review_Workflow(
			$this->repository,
			$this->validator,
			$approval_writer ?? new Post_Ingestion_Approval_Writer(),
			$this->embedding_generator
		);
	}
	...
	public function save_proposal( int $draft_id, array $input ) {
		return $this->review_workflow->save_proposal( $draft_id, $input );
	}
	...
	public function approve( int $draft_id, bool $approve_new_keywords ) {
		return $this->review_workflow->approve( $draft_id, $approve_new_keywords );
	}
