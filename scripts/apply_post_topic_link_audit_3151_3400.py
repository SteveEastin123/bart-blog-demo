"""Apply approved post-topic audit decisions for audit sequence 3151-3400."""
...
def main() -> None:
    ...
    applied_description_ids: set[str] = set()
    for entry in pending:
        ...
        if description_entry:
            ...
            applied_description_ids.add(wp_id)

        entry["status"] = "applied"
        entry["appliedAt"] = today

    # Description recommendations can exist even when a post needs no topic change.
    for description_entry in description_recommendations:
        wp_id = str(description_entry["wpId"])
        if wp_id in applied_description_ids:
            continue
        post = posts_by_id[wp_id]
        if post.get("description") != description_entry["descriptionBefore"]:
    ...
    note = (
        "Post-by-post topic-link audit for audit sequence "
        f"{START_SEQUENCE}-{END_SEQUENCE} updated this assignment."
    )
    ...
    print(
        ...
        f"{description_updates} description updates for audit sequence "
