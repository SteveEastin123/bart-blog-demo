"""Apply approved post-topic audit decisions for posts 1-100."""
...
DESCRIPTION_UPDATES = {
    "49926": (
        "Answers reader questions about historical evaluation of miracle claims, "
        "Paul's report of 500 resurrection witnesses, and the causes of Roman "
        "persecution at Lyon."
    ),
    "49716": (
        "Introduces the Blog Stewards seminar and provides a study guide to Romans "
        "1-3, focusing on Paul's argument, theology, and purpose in writing."
    ),
}
...
def main() -> None:
    ...
    if len(pending) != 4:
        raise ValueError(f"Expected four approved decisions, found {len(pending)}")
    ...
    for entry in pending:
        ...
        entry["appliedAt"] = date.today().isoformat()

    for wp_id, description in DESCRIPTION_UPDATES.items():
        posts_by_id[wp_id]["description"] = description
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 1-100 updated this assignment."
    ...
    for post_entry in pending:
        ...
        for topic in post_entry["topicsAdded"]:
        ...
        for topic in post_entry["topicsRemoved"]:
            topic_entry = topic_entries[topic]
            if topic_entry["status"] == "excluded":
                continue
            decisions = [
    ...
    post_tracker.update(
        {
            ...
            "appliedChangeCount": len(pending),
        }
    )
    ...
    print("Applied four topic decisions and two description updates.")
