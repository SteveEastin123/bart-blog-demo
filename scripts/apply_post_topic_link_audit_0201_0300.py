"""Apply approved post-topic audit decisions for posts 201-300."""
...
def main() -> None:
    ...
    if len(pending) != 12:
        raise ValueError(f"Expected 12 approved decisions, found {len(pending)}")
    if not all(201 <= entry["auditSequence"] <= 300 for entry in pending):
        raise ValueError("Found a pending decision outside posts 201-300")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 201-300 updated this assignment."
    ...
    print(
        f"Applied {len(pending)} topic decisions and "
        f"{description_updates} description updates for posts 201-300."
    )
