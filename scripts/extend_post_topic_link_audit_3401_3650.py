"""Record recommendations for the next 250 previously unaudited posts.
...
NOTES_PATH = ROOT / "data" / "audits" / "post_topic_link_audit_3401_3650_working_notes.md"
BATCH_SIZE = 250
...
RECOMMENDATIONS = {
    "8958": rec(
        ...
        reason="The post consists of a public video lecture on Forged and ancient literary forgery.",
    ),
    "8751": rec(
        add=("Media Interviews and Videos",),
        reason="The post is a public video lecture about the historical problems involved in studying Jesus.",
    ),
    "8776": rec(
        add=("Eternal Punishment",),
        reason="The full post gives a sustained critique of eternal torment, hell, final judgment, and the claim that unbelievers choose everlasting punishment.",
        description="Examines evangelical claims about final judgment and eternal punishment in response to former Moody classmates.",
    ),
    "8244": rec(
        remove=("Apostolic Fathers",),
        reason="The Apostolic Fathers appear only as supporting evidence within a post centrally concerned with when the canonical Gospels received their names.",
    ),
    "8087": rec(
        add=("Media Interviews and Videos",),
        reason="The post shares a Diane Rehm Show interview about Misquoting Jesus and textual changes in the Bible.",
    ),
    "7708": rec(
        add=("Media Interviews and Videos", "Textual Variants"),
        reason="The post is a public video lecture whose stated subject is Christian anti-Judaism in textual variants and apocryphal writings.",
    ),
}
...
def render_notes(entries: list[dict]) -> str:
    lines = [
        "# Post Topic Link Audit 3401-3650 Working Notes",
        "",
        "Canonical post records remain unchanged until the user approves this batch.",
        "",
        "## Progress",
        "",
        "- Audited 250 previously unaudited posts.",
...
def main() -> None:
    ...
    if len(tracker.get("posts", [])) != 3400:
        raise ValueError("Tracker must contain exactly 3400 audited posts")
    ...
    tracker.update(
        {
            ...
            "auditScope": "First 3650 stable audited posts in newest-first selection order",
