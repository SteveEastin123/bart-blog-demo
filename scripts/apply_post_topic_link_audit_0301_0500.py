"""Apply approved post-topic audit decisions for posts 301-500."""
...
def main() -> None:
    ...
    if len(pending) != 15:
        raise ValueError(f"Expected 15 approved decisions, found {len(pending)}")
    if not all(301 <= entry["auditSequence"] <= 500 for entry in pending):
        raise ValueError("Found a pending decision outside posts 301-500")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 301-500 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 301-500."
    )
