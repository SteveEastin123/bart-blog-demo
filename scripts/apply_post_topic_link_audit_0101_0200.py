"""Apply approved post-topic audit decisions for posts 101-200."""
...
def main() -> None:
    ...
    if len(pending) != 5:
        raise ValueError(f"Expected five approved decisions, found {len(pending)}")
    if not all(101 <= entry["auditSequence"] <= 200 for entry in pending):
        raise ValueError("Found a pending decision outside posts 101-200")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 101-200 updated this assignment."
    ...
    print("Applied five topic decisions for posts 101-200.")
