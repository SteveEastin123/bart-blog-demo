"""Apply approved post-topic audit decisions for posts 1101-1300."""
...
def main() -> None:
    ...
    if len(pending) != 41:
        raise ValueError(f"Expected 41 approved decisions, found {len(pending)}")
    if not all(1101 <= entry["auditSequence"] <= 1300 for entry in pending):
        raise ValueError("Found a pending decision outside posts 1101-1300")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 1101-1300 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 1101-1300."
    )
