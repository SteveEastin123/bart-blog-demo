"""Apply approved post-topic audit decisions for posts 701-900."""
...
def main() -> None:
    ...
    if len(pending) != 48:
        raise ValueError(f"Expected 48 approved decisions, found {len(pending)}")
    if not all(701 <= entry["auditSequence"] <= 900 for entry in pending):
        raise ValueError("Found a pending decision outside posts 701-900")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 701-900 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 701-900."
    )
