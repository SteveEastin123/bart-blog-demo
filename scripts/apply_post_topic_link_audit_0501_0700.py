"""Apply approved post-topic audit decisions for posts 501-700."""
...
def main() -> None:
    ...
    if len(pending) != 32:
        raise ValueError(f"Expected 32 approved decisions, found {len(pending)}")
    if not all(501 <= entry["auditSequence"] <= 700 for entry in pending):
        raise ValueError("Found a pending decision outside posts 501-700")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 501-700 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 501-700."
    )
