"""Apply approved post-topic audit decisions for posts 901-1100."""
...
def main() -> None:
    ...
    if len(pending) != 35:
        raise ValueError(f"Expected 35 approved decisions, found {len(pending)}")
    if not all(901 <= entry["auditSequence"] <= 1100 for entry in pending):
        raise ValueError("Found a pending decision outside posts 901-1100")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 901-1100 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 901-1100."
    )
