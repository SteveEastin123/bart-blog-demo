"""Apply approved post-topic audit decisions for posts 1301-1500."""
...
def main() -> None:
    ...
    if len(pending) != 42:
        raise ValueError(f"Expected 42 approved decisions, found {len(pending)}")
    if not all(1301 <= entry["auditSequence"] <= 1500 for entry in pending):
        raise ValueError("Found a pending decision outside posts 1301-1500")
    ...
    for topic in affected_topics:
        ...
        note = "Post-by-post topic-link audit for posts 1301-1500 updated this assignment."
    ...
    print(
        ...
        f"{description_updates} description updates for posts 1301-1500."
    )
